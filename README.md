...
### Regular training with FSDP or DeepSpeed
 We provide bash scripts that use PyTorch FSDP to finetune the 7B ([`train_camel_7B.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_7B.sh)) and 13B ([`train_camel_13B.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_13B.sh)) models on a single node with 4 and 8 A100-80GB GPUs respectively. We also include an example of how to finetune 13B model on multiple nodes ([`train_camel_13B_distributed.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_13B_distributed.sh)). Finally, if you have the resources, we provide a DeepSpeed example of how to finetune the 30B model on 16 A100-80GB GPUs with parameter and optimizer CPU offloading ([`train_camel_30B_distributed_deepspeed.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_30B_distributed_deepspeed.sh)).
...
### QLoRA training
If you do not have server grade GPUs, you may consider using QLoRA that makes it possible to finetune 13B model on consumer grade GPUs. We include an example script ([`train_camel_qlora.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_qlora.sh)) of how to use it. Note that our 30B and 65B models were finetuned with QLoRA with a global batch size of 256 on 4 A100-80GB GPUs. Huggingface does not support resuming from a checkpoint for QLoRA. We provide a way to fix that by adding an additional flag `load_weights=False` to `trainer.train()`. This will resume the optimizer, scheduler, and dataloader states but not the model weights since the base weight and adapter weights are loaded before training loop.

To resume finetuning from QLoRA checkpoint, add the flags `--resume_from_checkpoint True --checkpoint_path /path/to/adapter/file/checkpoint --load_weights False` when running [`train_camel_qlora.sh`](https://github.com/camel-ai/camel_chat/blob/main/scripts/train_camel_qlora.sh).
...
```
python -m camel_chat.model.download_camel_models --model CAMEL-13B-Combined-Data
```
# Serve
...
```
For example to download and host our model:
```
python -m camel_chat.model.download_camel_models --model CAMEL-13B-Combined-Data --download_directory finetuned_models
python -m camel_chat.serve.cli --model-path finetuned_models/CAMEL-13B-Combined-Data
```
### Serve in Web GUI
...
# Citation
Since this repository is built on top of [lm-sys/FastChat](https://github.com/lm-sys/FastChat) and [artidoro/qlora](https://github.com/artidoro/qlora/tree/main), if you used this code base in your project please cite the following:
